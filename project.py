...
def gconnect():
    if request.args.get('state') != login_session['state']:
        ...
        resopnse.headers['Content-Type'] = 'application/json'
        return response
    code = request.data
    try:
    ...
    url = ('htps://www.googleapis.com/oauth2/v1/tokeninfo?access_token=%s' % access_token)
    h = httplib2.Http()
    result = json.loads(h.request(url, 'GET')[1])

    # Abort if there's an error in teh access token info
    if result.get('error') is not None:
        response = make_response(json.dumps(result.get('error')), 500)
        response.headers['Content-Type'] = 'application/json'
        return response

    # Verify the token is used for the intended user
    ...
    if stored_credentials is not None and gplus_id == stored_gplus_id:
        ...
        response.headers['Content-Type'] = 'application/json'
        flash("Current user was already logged in")
        return response
    ...
    answer = requests.get(userinfo_url, parmas=params)
    ...
    # FIXME: flash message isn't appearing.
    return output
